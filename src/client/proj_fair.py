...
class ProjFairClient(Client):
    def __init__(self, cid, train_data, test_data, options):
        ...
        super(ProjFairClient, self).__init__(cid, train_data, test_data, options)
    ...
    def local_train(self):
        ...
        for local_round in range(self.num_local_round):
            ...
            self.local_model = self.local_model - self.local_lr * self.lamda * (cur_loss**self.q) * (self.local_model - proj_person_weight)
    ...
    #     flat_grads = self.lamda * (cur_loss**(self.q)) * (self.local_model - torch.matmul(self.Proj, self.person_model_params))
    #     return flat_grads
