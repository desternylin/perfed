...
class lp_proj_optimizer(grad_desc):
    ...
    def step(self, local_weight, Proj, closure = None):
        ...
        flat_regularizer = self.lamda * torch.matmul(Proj.t().data, torch.sign(conv_weight) * (torch.abs(conv_weight))**(self.p - 1))
        regularizer = set_flat_params_to_param_groups(regularizer, flat_regularizer)
...
class lp_proj_new_optimizer(grad_desc):
    def __init__(self, params, p, lr = required, lamda = 1.0, weight_decay = 0.0):
        self.p = p
        self.lamda = lamda

        super(lp_proj_new_optimizer, self).__init__(params, lr, weight_decay)
    ...
    def step(self, local_weight, Proj, closure = None):
        ...
        regularizer = copy.deepcopy(self.param_groups[0]['params'])
        flat_person_weight = get_flat_params_from_param_groups(regularizer)
        conv_weight = local_weight - torch.matmul(Proj.data, torch.matmul(Proj.t().data, flat_person_weight))
        flat_regularizer = self.lamda * torch.matmul(Proj.data, torch.matmul(Proj.t().data, torch.sign(conv_weight) * (torch.abs(conv_weight))**(self.p - 1)))
        regularizer = set_flat_params_to_param_groups(regularizer, flat_regularizer)
