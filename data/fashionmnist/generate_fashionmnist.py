...
def generate_for_task(equal = True):
    # Get Fashion-MNIST data, normalize, and divide by level
    print('>>> Get Fashion-MNIST data.')
    ...
    trainset = torchvision.datasets.FashionMNIST(DATASET_FILE, download = True, train = True, transform = transform)
    testset = torchvision.datasets.FashionMNIST(DATASET_FILE, download = True, train = False, transform = transform)
    train_fmnist = ImageDataset(trainset.train_data, trainset.train_labels)
    test_fmnist = ImageDataset(testset.test_data, testset.test_labels)

    # Sort the data according to their classes
    fmnist_traindata = []
    for number in range(10):
        idx = train_fmnist.target == number
        fmnist_traindata.append(train_fmnist.data[idx])

    if equal:
        min_number = min([len(dig) for dig in fmnist_traindata])
        # Force each digit to have the same amount of train data
        for number in range(10):
            fmnist_traindata[number] = fmnist_traindata[number][:min_number-1]

    split_fmnist_traindata = []
    for digit in fmnist_traindata:
        split_fmnist_traindata.append(data_split(digit, int(NUM_USER * 2 / 10)))

    fmnist_testdata = []
    for number in range(10):
        idx = test_fmnist.target == number
        fmnist_testdata.append(test_fmnist.data[idx])
    split_fmnist_testdata = []
    for digit in fmnist_testdata:
        split_fmnist_testdata.append(data_split(digit, int(NUM_USER * 2 / 10)))

    data_distribution = np.array([len(v) for v in fmnist_traindata])
    ...
    digit_count = np.array([len(v) for v in split_fmnist_traindata])
    print('>>> Each digit in train data is split into {}'.format(digit_count.tolist()))

    digit_count = np.array([len(v) for v in split_fmnist_testdata])
    ...
    for user in range(NUM_USER):
        print(user, np.array([len(v) for v in split_fmnist_traindata]))
            
        for d in choose_digit(split_fmnist_traindata, 2):
            l = len(split_fmnist_traindata[d][-1])
            X = split_fmnist_traindata[d].pop().tolist()
            valid_len = int(0.2 * l)
            train_len = l - valid_len
            # train_X[user] += split_fmnist_traindata[d].pop().tolist()
            ...
            l = len(split_fmnist_testdata[d][-1])
            test_X[user] += split_fmnist_testdata[d].pop().tolist()
            test_y[user] += (d * np.ones(l)).tolist()

    # Setup directory for train/test data
    print('>>> Set data path for Fashion-MNIST')
